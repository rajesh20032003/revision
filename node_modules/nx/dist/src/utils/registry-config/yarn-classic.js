...
exports.getYarnClassicSpawnRegistryEnv = getYarnClassicSpawnRegistryEnv;
...
function getYarnClassicSpawnRegistryEnv(packageName, root) {
    const env = {};
    const scope = (0, utils_1.getPackageScope)(packageName);
    const realHome = (0, os_1.homedir)();
    const { primary, secondary } = yarnHomeTiers(realHome);
    const ancestors = (0, utils_1.ancestorDirectories)(root);
    ...
    const sources = [
        ...
        {
            npmrcPath: (0, path_1.join)(etcDir, 'npmrc'),
            yarnrcPath: (0, path_1.join)(etcDir, 'yarnrc'),
            npmNative: true,
        },
        ...(secondary ? [dotfiles(secondary.dir, secondary.npmNative)] : []),
        ...ancestors.map((dir) => dotfiles(dir, false)),
    ];
    const npmrcChain = sources.map((s) => ({
    ...
    const yarnrcChain = sources.map((s) => ({
        // npm never reads .yarnrc.
        npmNative: false,
        map: readYarnrcMap(s.yarnrcPath, ungatedPaths.has(s.yarnrcPath) ? 'both' : 'looked-up'),
    }));
    const cliRegistryChain = cliRcPaths.map((rcPath) => ({
        npmNative: false,
        map: readYarnrcMap(rcPath, lookedUpPaths.has(rcPath) ? 'both' : 'cli-rc'),
    }));
    const authRegistry = resolveRegistry(env, npmrcChain, yarnrcChain, cliRegistryChain, scope);
...
function resolveAuth(env, npmrcChain, scope, authRegistry) {
    const prefixes = registryOptionPrefixes(authRegistry);
    const requestDart = (0, utils_1.requestNerfDart)(authRegistry);
    ...
    if (!authenticates) {
        if (requestDart) {
            (0, utils_1.warnNativeCredential)(env, requestDart, 'yarn', 'yarn does send it for scoped packages, and for any registry with always-auth set, so removing it from .npmrc would stop those from authenticating too.', (key) => {
                const match = firstString(npmrcChain, key);
                return ((0, utils_1.readNpmConfigEnv)(process.env, key) ??
    ...
    if (!winner.values.every((match) => match.prefix === requestDart && match.npmNative)) {
    ...
    for (const dart of gatedAuthDarts(npmrcChain, requestDart)) {
        ...
        for (const key of outranking) {
            ...
            const native = (0, utils_1.readNpmConfigEnv)(process.env, `${dart}:${key}`) ??
...
function npmNativeValue(npmrcChain, key) {
 ...
 */
function gatedAuthDarts(npmrcChain, requestDart) {
    const gates = [];
    const declared = [];
    for (const file of npmrcChain) {
        if (!file.map) {
    ...
    for (const gate of gates) {
        for (const dart of [gate, ...declared]) {
            if (dart.matchHost !== gate.matchHost ||
 ...
 */
function registryOptionPrefixes(registry) {
    const prefixes = [];
...
function resolveCredential(prefixes, npmrcChain, key) {
...
function alwaysAuthFor(prefixes, npmrcChain) {
    for (const prefix of prefixes) {
...
function yarnConfigValue(key, envName, npmrcChain) {
    if (envName !== null) {
        const fromEnv = (0, utils_1.readEnvVar)(process.env, `npm_config_${envName}`) ??
            (0, utils_1.readEnvVar)(process.env, `yarn_${envName}`);
...
function normalizeYarnConfigValue(value) {
...
function resolveRegistry(env, npmrcChain, yarnrcChain, cliYarnrcChain, scope) {
    const scopedRegistry = scope
        ? resolveScopedRegistry(env, npmrcChain, yarnrcChain, scope)
        : undefined;
    const unscopedRegistry = resolveUnscopedRegistry(env, npmrcChain, yarnrcChain, cliYarnrcChain);
    if (scope) {
        pinScopedRegistry(env, npmrcChain, scope, scopedRegistry ?? unscopedRegistry);
    }
...
function pinScopedRegistry(env, npmrcChain, scope, effectiveRegistry) {
    const scopedKey = `${scope}:registry`;
    if (env[`npm_config_${scopedKey}`] !== undefined) {
        return;
    }
    const npmReads = (0, utils_1.readNpmConfigEnv)(process.env, scopedKey) ??
    ...
    if (npmReads !== undefined && npmReads !== effective) {
        (0, utils_1.setScopedRegistry)(env, scope, effective);
    }
}
function resolveUnscopedRegistry(env, npmrcChain, yarnrcChain, cliYarnrcChain) {
    ...
    const cliRegistry = firstString(cliYarnrcChain, '--install.registry') ??
        firstString(cliYarnrcChain, '--registry');
    if (cliRegistry) {
        (0, utils_1.setRegistry)(env, cliRegistry.value);
        return cliRegistry.value;
    }
    // 2. npm_config_registry env: npm resolves it natively.
    const npmConfigRegistry = (0, utils_1.readEnvVar)(process.env, 'npm_config_registry');
    if (npmConfigRegistry !== undefined) {
        return npmConfigRegistry;
    }
    // 3. YARN_REGISTRY env (yarn-only).
    const yarnRegistryEnv = (0, utils_1.readEnvVar)(process.env, 'YARN_REGISTRY');
    if (yarnRegistryEnv !== undefined) {
        (0, utils_1.setRegistry)(env, yarnRegistryEnv);
    ...
    if (npmrcRegistry) {
        if (!npmrcRegistry.npmNative) {
            (0, utils_1.setRegistry)(env, npmrcRegistry.value);
        }
    ...
    if (yarnrcRegistry &&
        ...
        (0, utils_1.setRegistry)(env, yarnrcRegistry.value);
...
function readYarnMergedEnv(prefix, key) {
    if (key.includes('.')) {
        return undefined;
    }
    let value;
...
function resolveScopedRegistry(env, npmrcChain, yarnrcChain, scope) {
    const scopedKey = `${scope}:registry`;
    ...
    if (npmTierEnvRegistry) {
        if (npmConfigEnvRegistry) {
            ...
            if ((0, utils_1.readNpmConfigEnv)(process.env, scopedKey) !== npmConfigEnvRegistry) {
                (0, utils_1.setScopedRegistry)(env, scope, npmConfigEnvRegistry);
            }
        }
        else {
            (0, utils_1.setScopedRegistry)(env, scope, npmTierEnvRegistry);
        }
    ...
    if (npmTierEnvRegistry === undefined) {
        ...
        if (npmScoped) {
            if (!npmScoped.npmNative) {
                (0, utils_1.setScopedRegistry)(env, scope, npmScoped.value);
            }
    ...
    if (yarnEnvRegistry) {
        (0, utils_1.setScopedRegistry)(env, scope, yarnEnvRegistry);
        return yarnEnvRegistry;
    }
    if (yarnEnvRegistry === undefined) {
        const yarnScoped = firstString(yarnrcChain, scopedKey);
        if (yarnScoped) {
            (0, utils_1.setScopedRegistry)(env, scope, yarnScoped.value);
            return yarnScoped.value;
        }
    }
    return undefined;
}
function resolveOptions(env, npmrcChain, yarnrcChain, root, home) {
    const cafile = yarnEnvOption('cafile') ??
        resolveOption(firstString, npmrcChain, yarnrcChain, 'cafile');
    if (cafile) {
        (0, utils_1.setCafile)(env, resolveYarnPath(cafile, root, home));
    }
    ...
    const strictSsl = strictSslEnv !== undefined
    ...
    if (strictSsl !== undefined &&
        ...
        (0, utils_1.setStrictSsl)(env, truthyStrictSsl(strictSsl));
    }
    (0, utils_1.setProxies)(env, {
        httpProxy: yarnEnvOption('proxy') ??
            resolveOption(firstString, npmrcChain, yarnrcChain, 'proxy'),
        httpsProxy: yarnEnvOption('https-proxy') ??
 ...
 */
function yarnEnvOption(key) {
    return (0, utils_1.readEnvVar)(process.env, `yarn_${key.replace(/-/g, '_')}`);
}
...
// undefined.
function resolveOption(lookup, npmrcChain, yarnrcChain, key) {
    ...
    if ((0, utils_1.readNpmConfigEnv)(process.env, key) !== undefined) {
        return undefined;
    }
    const npmrc = lookup(npmrcChain, key);
    return npmrc && !npmrc.npmNative ? npmrc.value : undefined;
}
function dotfiles(dir, npmNative) {
    return {
        npmrcPath: (0, path_1.join)(dir, '.npmrc'),
        yarnrcPath: (0, path_1.join)(dir, '.yarnrc'),
        npmNative,
    };
}
 ...
 */
function yarnCliRcPaths(root, ancestors, primaryHome) {
    const paths = [];
    ...
    if (home) {
        paths.push((0, path_1.join)(home, '.yarnrc.yml'), (0, path_1.join)(home, '.yarnrc'), (0, path_1.join)(home, '.yarn', 'config'), (0, path_1.join)(home, '.config', 'yarn'), (0, path_1.join)(home, '.config', 'yarn', 'config'), yarnConfigDir(primaryHome));
    }
    if (process.platform !== 'win32') {
        // The literal /etc, not the <prefix>/etc tier the registry client reads.
        paths.push((0, path_1.join)('/etc', 'yarnrc'), (0, path_1.join)('/etc', 'yarn', 'config'));
    }
    // A .yarnrc.yml sibling rides along with every .yarnrc yarn names, and lands
    // above it.
    for (const dir of [root, ...ancestors]) {
        paths.push((0, path_1.join)(dir, '.yarnrc.yml'), (0, path_1.join)(dir, '.yarnrc'));
    }
    return paths;
}
...
function yarnConfigDir(primaryHome) {
    if (process.platform === 'win32') {
        return process.env.LOCALAPPDATA
            ? (0, path_1.join)(process.env.LOCALAPPDATA, 'Yarn', 'Config')
            : (0, path_1.join)(primaryHome, '.config', 'yarn');
    }
    return process.env.XDG_CONFIG_HOME
        ? (0, path_1.join)(process.env.XDG_CONFIG_HOME, 'yarn')
        : (0, path_1.join)(primaryHome, '.config', 'yarn');
}
// Mirrors yarn's getGlobalPrefix.
function globalEtcDir() {
    ...
    if (process.env.PREFIX) {
        return (0, path_1.join)(process.env.PREFIX, 'etc');
    }
    if (process.platform === 'win32') {
        return (0, path_1.join)((0, path_1.dirname)(process.execPath), 'etc');
    }
    const prefix = (0, path_1.dirname)((0, path_1.dirname)(process.execPath));
    // DESTDIR reroots the prefix on Unix only, and only once PREFIX has passed.
    return (0, path_1.join)(process.env.DESTDIR ? (0, path_1.join)(process.env.DESTDIR, prefix) : prefix, 'etc');
}
...
function truthyStrictSsl(value) {
    return Boolean(value);
}
// Only a literal `false` turns npm's own strict-ssl off, and toYarnValueMap has
// already typed that as a boolean.
function npmVerifiesTls(npmrcChain) {
    const fromEnv = (0, utils_1.readNpmConfigEnv)(process.env, 'strict-ssl');
    if (fromEnv !== undefined) {
        return fromEnv !== 'false';
    }
    const native = firstDefined(npmrcChain.filter((file) => file.npmNative), 'strict-ssl');
...
function resolveYarnPath(value, root, home) {
    if (value === '~') {
        return home;
    }
    if (value.startsWith('~/') || value.startsWith('~\\')) {
        return (0, path_1.resolve)(home, value.slice(2));
    }
    return (0, path_1.resolve)(root, value);
}
...
function readChainNpmrcMap(path) {
...
function toYarnValueMap(map) {
    ...
    for (const [key, value] of map) {
        result.set(key, normalizeYarnConfigValue((0, utils_1.expandYarnEnvVars)(value)));
    }
    return result;
}
/**
 ...
 */
function parseYarnrcAsYaml(path) {
    let loaded;
    try {
        loaded = (0, fileutils_1.readYamlFile)(path, { failsafe: true });
    }
    catch {
