...
function needsShell(binary) {
    ...
    const ext = path_1.win32.extname(binary).toLowerCase();
 ...
 */
function safeSpawn(binary, args, options = {}) {
    const shell = needsShell(binary);
    return (0, child_process_1.spawn)(quoteBinary(binary, shell), quoteArgs(args, shell), {
        ...options,
        windowsHide: true,
        shell,
    });
 ...
 */
function safeExecFileSync(binary, args, options = {}) {
    const shell = needsShell(binary);
    return (0, child_process_1.execFileSync)(quoteBinary(binary, shell), quoteArgs(args, shell), {
        stdio: 'pipe',
        ...options,
        windowsHide: true,
        shell,
...
function quoteForCmd(value, described, remedy = '') {
    if (LINE_BREAK.test(value)) {
        throw new Error(`Cannot pass ${described} to cmd.exe: a line break inside it would end the command line before cmd.exe reached the rest of it.${remedy}`);
    }
    return (0, shell_quoting_1.quoteShellArg)(value);
}
function quoteArgs(args, shell) {
    if (!shell) {
        return [...args];
    }
    return args.map((arg) => quoteForCmd(arg, `the argument ${JSON.stringify(arg)}`, ' Remove it from your Nx configuration and try again.'));
}
