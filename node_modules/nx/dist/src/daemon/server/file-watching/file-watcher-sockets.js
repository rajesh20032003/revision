...
exports.notifyFileWatcherSocketsOfError = notifyFileWatcherSocketsOfError;
 ...
 */
function notifyFileWatcherSocketsOfError(error) {
    if (!hasRegisteredFileWatcherSockets()) {
        return;
    }
    queue.sendToQueue(async () => {
        await Promise.all(exports.registeredFileWatcherSockets.map(({ socket }) => (0, server_1.handleResult)(socket, 'FILE-WATCH-CHANGED', () => Promise.resolve({
