...
class ReleaseGroupProcessor {
    constructor(tree, projectGraph, nxReleaseConfig, releaseGraph, options) {
         ...
         */
        this.allVersionDecisionsFinalized = false;
        /**
    ...
    async updateDependenciesForProject(projectName) {
        ...
        const projectLogger = this.getProjectLoggerForProject(projectName);
        const logMessages = await versionActions.updateProjectDependencies(this.tree, this.projectGraph, dependenciesToUpdate, (dependencyProjectName) => this.resolveVersionForDependency(dependencyProjectName));
        for (const logMessage of logMessages) {
            projectLogger.buffer(logMessage);
        }
    }
    finalizeVersionDecisionsForGroup(releaseGroup) {
        this.groupsWithFinalizedVersionDecisions.add(releaseGroup.name);
    }
    async finalizeAllVersionDecisions(processOrder) {
        const unfinalizedReleaseGroups = Array.from(this.releaseGraph.groupGraph.keys()).filter((releaseGroupName) => !this.groupsWithFinalizedVersionDecisions.has(releaseGroupName));
        if (unfinalizedReleaseGroups.length > 0) {
            const releaseGroupNames = unfinalizedReleaseGroups
                ...
                .join(', ');
            throw new Error(`Unable to finalize version decisions for release groups ${releaseGroupNames}. Please report this as a bug on https://github.com/nrwl/nx/issues.`);
        }
        ...
        if (this.projectsWithDeferredDependencyUpdates.size > 0) {
            const deferredProjects = Array.from(this.projectsWithDeferredDependencyUpdates, (projectName) => `"${projectName}"`).join(', ');
            throw new Error(`Unable to update dependencies for projects ${deferredProjects} because their release groups were not processed. Please report this as a bug on https://github.com/nrwl/nx/issues.`);
        }
        this.flushAllProjectLoggers();
    }
    /**
     ...
     * @internal
     */
    async resolveVersionForDependency(dependencyProjectName) {
        ...
        if (!this.releaseGraph.allProjectsToProcess.has(dependencyProjectName)) {
            return this.releaseGraph.resolveCurrentVersionForDependency(this.tree, this.projectGraph, dependencyProjectName, this.options.preid);
        }
        const releaseGroupName = this.getReleaseGroupNameForProject(dependencyProjectName);
        if (!releaseGroupName ||
            !this.groupsWithFinalizedVersionDecisions.has(releaseGroupName)) {
            throw new Error(`Cannot resolve a concrete version for dependency project "${dependencyProjectName}" because its release group has not finalized its version decisions. Ensure the release group can finalize the dependency version before Nx Release updates the dependent manifest.`);
        }
        if (versionData?.currentVersion) {
            return versionData.currentVersion;
        }
        throw new Error(`Cannot resolve a concrete version for dependency project "${dependencyProjectName}" because its finalized release data has no current or new version.`);
    }
