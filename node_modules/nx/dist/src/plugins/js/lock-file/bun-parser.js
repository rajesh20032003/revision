...
function parseLockFile(lockFileContent, lockFileHash) {
    ...
    try {
        ...
        if (result.lockfileVersion !== undefined) {
            ...
            if (!Number.isInteger(result.lockfileVersion) ||
                result.lockfileVersion < 0) {
                throw new Error(`Unsupported lockfile version ${result.lockfileVersion}. Lockfile version must be a non-negative integer`);
            }
            if (result.lockfileVersion > MAX_KNOWN_LOCKFILE_VERSION) {
                console.warn(`bun.lock has lockfileVersion ${result.lockfileVersion}, which is newer than the version ${MAX_KNOWN_LOCKFILE_VERSION} this version of Nx was tested with. Attempting to parse it anyway.`);
            }
        }
...
function processPackageForDependencies(packageKey, packageData, lockFile, index, ctx, workspacePackages) {
    ...
    for (const depType of DEPENDENCY_TYPES) {
        ...
        if (!deps || typeof deps !== 'object')
            continue;
        const depDependencies = processDependencyEntries(deps, packageKey, sourceNodeName, lockFile, index, ctx, workspacePackages);
...
function processDependencyEntries(deps, sourcePackageKey, sourceNodeName, lockFile, index, ctx, workspacePackages) {
    const dependencies = [];
    const depsEntries = Object.entries(deps);
    for (const [packageName, versionSpec] of depsEntries) {
        try {
            const dependency = processSingleDependency(packageName, versionSpec, sourcePackageKey, sourceNodeName, lockFile, index, ctx, workspacePackages);
            if (dependency) {
...
function processSingleDependency(packageName, versionSpec, sourcePackageKey, sourceNodeName, lockFile, index, ctx, workspacePackages) {
    ...
    const nestedVersion = resolveNestedVersion(sourcePackageKey, packageName, lockFile);
    ...
    if (nestedVersion) {
        ...
        const resolvedVersion = findResolvedVersion(packageName, versionSpec, index, lockFile.manifests);
        if (!resolvedVersion) {
            return null;
        }
...
function resolveNestedVersion(sourcePackageKey, packageName, lockFile) {
