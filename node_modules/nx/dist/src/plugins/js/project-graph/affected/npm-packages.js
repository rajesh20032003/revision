...
const getTouchedNpmPackages = (touchedFiles, _, nxJson, packageJson, projectGraph) => {
    ...
    for (const c of changes) {
        if ((0, json_diff_1.isJsonChange)(c) &&
            ...
            const matchingNpmPackages = findPackagesForSelector(packageSelector, packagesByName, c.path[0] === 'pnpm');
            ...
            if (matchingNpmPackages.some((pkg) => globalPackages.has(pkg.data.packageName))) {
                return Object.keys(projectGraph.nodes);
            }
...
function getPackageSelector(change) {
    if (typeof change.value.lhs !== 'string' &&
...
function groupPackagesByName(npmPackages) {
...
function findPackagesForSelector(selector, packagesByName, isPnpmOverride) {
    if (isPnpmOverride) {
    ...
    const packageName = selector.match(/(?:^|\/)(@[^/@>\s]+\/[^/@>\s]+|[^/@>\s]+)(?:@[^/]*)?$/)?.[1];
