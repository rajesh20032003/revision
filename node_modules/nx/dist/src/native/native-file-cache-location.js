...
exports.NATIVE_CACHE_ROOT = void 0;
exports.getNativeFileCacheLocationToDelete = getNativeFileCacheLocationToDelete;
exports.ensureSecureNativeFileCacheLocation = ensureSecureNativeFileCacheLocation;
 ...
 */
exports.NATIVE_CACHE_ROOT = (0, path_1.join)(nx_tmp_dir_1.NX_USER_TMP_DIR, 'native-cache');
/**
...
function getNativeFileCacheLocationToDelete() {
    ...
    if (configured !== undefined) {
        ...
        return (0, owned_private_dir_1.isOwnedRealDirectory)(configured) ? configured : null;
    }
    return (0, owned_private_dir_1.isSafeSharedRoot)(nx_tmp_dir_1.NX_TMP_DIR).status === 'ok' &&
        (0, owned_private_dir_1.isOwnedRealDirectory)(nx_tmp_dir_1.NX_USER_TMP_DIR) &&
        (0, owned_private_dir_1.isOwnedRealDirectory)(exports.NATIVE_CACHE_ROOT)
        ? (0, path_1.join)(exports.NATIVE_CACHE_ROOT, versions_1.nxVersion)
        : null;
}
/**
 ...
 * planted before us.
 */
function ensureSecureNativeFileCacheLocation(
// Test seam: lets a spec plant a hostile directory under a root it controls.
cacheRoot = exports.NATIVE_CACHE_ROOT) {
    const configured = configuredCacheDir();
    if (configured !== undefined) {
        ...
        try {
            ...
            (0, fs_1.mkdirSync)((0, path_1.dirname)(dir), { recursive: true });
            const established = (0, owned_private_dir_1.ensureOwnedPrivateDir)(dir);
            if (established.status !== 'ok') {
                ...
                throw new owned_private_dir_1.DirectoryRefusedError(established.refusal);
            }
            return dir;
        }
        catch (e) {
            ...
            console.warn(`Nx could not use the configured native file cache directory ${dir} (${e?.code ?? e?.message ?? e}). Loading the native binding in place instead.`);
            return null;
        }
    }
    const userRoot = (0, path_1.dirname)(cacheRoot);
    const sharedRoot = (0, path_1.dirname)(userRoot);
    ...
    if ((0, owned_private_dir_1.ensureSafeSharedRoot)(sharedRoot).status !== 'ok') {
        return null;
    }
    for (const root of [userRoot, cacheRoot]) {
        if ((0, owned_private_dir_1.ensureOwnedPrivateDir)(root).status !== 'ok') {
            return null;
        }
    }
    // Verified, not assumed: this is the directory we load a `.node` out of.
    const versionDir = (0, path_1.join)(cacheRoot, versions_1.nxVersion);
    if ((0, owned_private_dir_1.ensureOwnedPrivateDir)(versionDir).status !== 'ok') {
        return null;
    }
    return versionDir;
}
